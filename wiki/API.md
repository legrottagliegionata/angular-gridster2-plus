...
| `getNextPossiblePosition(item, startingFrom?)` | `boolean`                   | Find a free position for `item` and write it into `item.x`/`item.y`. Without `startingFrom`, an item already on the grid in a valid position keeps it. `startingFrom: { x?, y? }` starts the search from that cell; the following rows are searched from the first column. Returns `false` when there is no room |
