...
Scrolling the grid yourself during a drag or a resize, with the wheel or a trackpad, also keeps the item or its edge under the pointer. A drag the browser turns into a native one, which happens when the pointer starts on selected text, is stopped instead of leaving the item floating.
